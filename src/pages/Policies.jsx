    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "We will never ask you to leave your device without an official receipt. Always keep the invoice and warranty card for follow-ups.",
        ],
      },
      {
        id: "cancellation",
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "لن نطلب منك أبدًا ترك الجهاز دون إيصال رسمي. احتفظ بالفاتورة وكارت الضمان للمتابعة.",
        ],
      },
      {
        id: "cancellation",
...
export default function Policies() {
  ...
  // Smooth scroll to target, relying on CSS scroll-margin-top
  const scrollToId = (id) => {
  ...
  // Open section & scroll when hash present/changes
  useEffect(() => {
    if (!hash) return;
    ...
    // slight delay to ensure panel renders
  ...
  const onTocClick = (e, id) => {
    ...
    // optional immediate scroll (in case router doesn't trigger effect quickly)
    setTimeout(() => scrollToId(id), 0);
  };

  // SEO metadata (React 19 document metadata)
  const title = isEn
    ? "Policies & Privacy | American Group"
    : "السياسات والخصوصية | الأمريكية جروب";
  const desc = isEn
    ? "Service policies, terms, and privacy practices."
    : "سياسات الخدمة والشروط وممارسات الخصوصية.";
  const canonical =
    "https://americangroup-eg.com" + (isEn ? "/en/policies" : "/policies");
        ...
        >
          {t.sections.map((s) => (
            <AccordionItem key={s.id} border="none" mb={2}>
              {/* ✅ عنصر المرساة له scroll-margin-top بدل top:-80px */}
              <Box id={s.id} sx={{ scrollMarginTop: "90px" }} />
              <h3>
                <AccordionButton
                  px={4}
                  py={3}
                  borderRadius="lg"
                  _expanded={{ bg: "blue.50", _dark: { bg: "whiteAlpha.100" } }}
                >
                  <Box
                    as="span"
                    flex="1"
