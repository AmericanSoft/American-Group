...
export default function Policies() {
  ...
  // افتح السيكشن المطلوب لو فيه ?sec=...
  const section = searchParams.get("sec");
  useEffect(() => {
    if (!section) return;
    const idx = idToIndex[section];
    if (idx === undefined) return;

    setExpanded((prev) =>
      Array.from(new Set([...(Array.isArray(prev) ? prev : []), idx]))
    );
  ...
  const onTocClick = (e, id) => {
    ...
    // حدّث الـ query بدل الـ hash
    setSearchParams({ sec: id }, { replace: false });
  };
